...
//     try {
//       const token = localStorage.getItem('token');
//       // Updated endpoint from /export to /download to match the router
...
const ReportsList = () => {
  ...
  useEffect(() => {
    fetchReports();
    fetchIncidentCount();
  }, [filters.search, filters.incidentType, filters.threatLevel]);
  ...
  return (
    <div className="min-h-screen bg-black text-green-400 p-6">
      ...
      <div className="container mx-auto max-w-7xl">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-green-300">Incident Reports</h1>
            <p className="text-green-600">Total Reports: {pagination.totalReports}</p>
          </div>
          <div className="flex space-x-4">
            <button 
              className="bg-green-700 text-white px-4 py-2 rounded-md flex items-center hover:bg-green-600 transition"
            ...
            >
              <BarChart2 className="mr-2" />
              Incidents 
              <span className="ml-2 bg-green-500 text-white rounded-full px-2 py-1 text-xs">{incidentCount}</span>
            </button>
          </div>
        </div>
        ...
        {!loading && filteredReports.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredReports.map((report) => (
              <div 
                key={report._id}
                className="bg-gray-900 border border-green-700 rounded-lg p-6 transform transition hover:scale-105 hover:shadow-2xl cursor-pointer"
                onClick={() => navigate(`/reports/${report._id}`)}
              >
                <div className="flex justify-between items-center mb-4">
                  <span className="text-xl font-bold text-green-400">{report.Report_ID || report._id}</span>
                  <span className={`
                ...
                <h3 className="text-lg font-semibold text-green-300 mb-2">{report.Incident_Type}</h3>
                <p className="text-green-500 mb-3 line-clamp-2">{report.Description}</p>

                <div className="text-sm text-green-500 mb-1">📍 {report.Location}</div>
                <div className="text-sm text-green-500 mb-1">
                  📅 {new Date(report.Date).toLocaleDateString()} — 🕒 {new Date(report.Date).toLocaleTimeString()}
                </div>

                {report.Status && (
                  <div className="text-sm text-green-400 mb-1">Status: {report.Status}</div>
                )}
                {report.Reported_By && (
                  <div className="text-sm text-green-400">Reported By: {report.Reported_By}</div>
                )}
              </div>
            ))}
          </div>
          ...
          !loading && (
            <div className="text-center text-green-500 mt-20 text-xl">
